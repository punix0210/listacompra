...
export class DatabaseService {
  ...
  public async createDataBase() {
    ...
    try {
      ...
      console.log(`Estou mostrando o banco ${conn}`);
  ...
  private async createtable(db: SQLiteObject) {
    ...
    try {
      ...
      console.log(`Estou mostrando o sql ${sql}`);

      const table = await this.sqlitePorter.importSqlToDb(db, sql);

      console.log(`Estou mostrando resultado da importação ${table}`);
