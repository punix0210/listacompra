...
export class ListaService {
  ...
  async save(lista: Lista) {
    const conn = await this.dbService.getDB();
    if (lista.id === 0) {
      const sql = 'insert into Lista (descricao, data, vlPrevisto) values (?, ?, ?, ?, ?)';
      const data = [lista.descricao, lista.data, lista.vlPrevisto];
      return await conn.executeSql(sql, data);
    } else {
      const sql = 'update Lista set descricao=? data=? vlPrevisto=? Where id=?';
  ...
  async findById(id) {
    const conn = await this.dbService.getDB();
    try {
      const sql = 'Select * from Lista Where id=?';
      const data = [id];
