  ...
  findById(id): Promise<Response> {
    return new Promise(async (resolve) => {
      try {
        ...
        let lista = new Lista(0, '', Lib.dateToString(new Date()), 0.00);
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (lista.id === 0) {
          ...
          const sql = 'update ListaDespesa set descricao=?, vlPrevisto=? Where id=?';
          const data = [lista.descricao, lista.vlPrevisto, lista.id];
          const resp = await conn.executeSql(sql, data);
